USAGE:
user@host:~ $ python ws3000-standalone_r.py
{'dateTime': 1581659527, 'extraHumid6': 30, 'extraHumid7': 33, 'extraHumid4': 33                                                                                                                                                                                                                                    , 'extraHumid5': 30, 'extraHumid2': 30, 'extraHumid3': 31, 'extraHumid1': 92, 'e                                                                                                                                                                                                                                    xtraTemp6': 18.1, 'extraTemp7': 21.8, 'extraTemp4': 20.1, 'extraTemp5': 21.2, 'e                                                                                                                                                                                                                                    xtraTemp2': 20.9, 'extraTemp3': 20.6, 'usUnits': 17, 'extraTemp1': -1.2}
user@host:~

CODE: (save as ws3000-standalone_r.py)
...
THIS IS A STRIPPED STANDALONE VERSION!!

RUN: python ws3000-standalone_r.py
...
class WS3000():
    ...
    def open_port(self):
        ...
        if not self.device:
            ...
            raise IOError("Unable to find USB device")
    ...
    def get_current_values(self):
        ...
        if self.mode == 'simulation':
            ...
            new_packet = {'dateTime': int(current_time), 'usUnits': METRICWX}
        ...
        while nberrors < self.max_tries:
            ...
            try:
                ...
                return new_packet
            except (usb.USBError, IOError) as e:
        ...
        raise usb.USBError("Max retries exceeded while fetching USB reports")
    ...
    def _get_raw_data(self, hex_command=COMMANDS['sensor_values']):
        ...
        try:
            ...
            logerr(exc_traceback)
            traceback.print_exc(file=sys.stdout)
            raise IOError("Error while fetching " + self._get_cmd_name(hex_command))
    ...
    def _raw_to_data(self, buf, hex_command=COMMANDS['sensor_values']):
        ...
        if hex_command == self.COMMANDS['sensor_values']:
            if len(buf) != 27:
                raise IOError("Incorrect buffer length, failed to read " + self._get_cmd_name(hex_command))
    ...
    def _data_to_wxpacket(self, station_data):
        ...
        new_packet = {'dateTime': int(time.time() + 0.5), 'usUnits': METRICWX}
